	...
	cyl = (cylinder(r=miniservo_width/2, h=3, yaw=deg(180))
		.rotateY(deg(90)).rotateX(deg(90))
			.forw(miniservo_width/2)
				.up(miniservo_length+thickness)
	)
	ncyl = cylinder(r=3, h=8) 
	ncyl2 = cylinder(r=1, h=8)
	ncyl_trans = up(miniservo_length+thickness) * forw(miniservo_width/2) * right(thickness) * rotateY(deg(90))
	b = box(thickness, miniservo_width, miniservo_length+thickness)

	b2 = box(8 + thickness, thickness, miniservo_length+thickness).forw((miniservo_width - thickness)/2)
	b3 = box(2, thickness, miniservo_length+thickness-2.5).forw((miniservo_width - thickness)/2).right(8 + thickness - 2)
